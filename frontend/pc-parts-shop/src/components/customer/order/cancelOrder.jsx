...
const CancelOrder = ({order, employeeId, employeeName, hide}) => {
  const dispatch = useDispatch();
  const handleCancelOrder = () => {
    if(order.status === "CHOXACNHAN") dispatch(cancelOrderStart({id: order.id, employeeId, employeeName, hide}));
    else  dispatch(updateStatusOrderStart({ orderId: order.id, status: "DAHUY", hide }));
  }

  return (
    <div className='py-5 px-20 flex flex-col gap-3 justify-center items-center shadow-md overflow-hidden w-[600px]'>
          <div className='text-center'>Xác nhận hủy đơn hàng <span className='font-semibold '>#{order.id}</span></div>
