...
import { activeEmployeeStart, deleteEmployeeStart } from '../../../redux/slices/employeeSlice';
import { deleteCustomerStart } from '../../../redux/slices/customerSlice';

const DeleteCustomer = ({customer, hide}) => {
  const dispatch = useDispatch();

  const handleDeleteCustomer= () => {
    dispatch(deleteCustomerStart({customerId: customer.customerId,  hide}));
  }

  return (
    <div className='py-5 px-20 flex flex-col gap-3 justify-center items-center shadow-md overflow-hidden w-[600px]'>
          <div className='text-center'>Bạn chắc chắn muốn xóa khách hàng <span className='font-semibold '>#{customer.customerId} - {customer.lastName + " " + customer.firstName} ?</span></div>
