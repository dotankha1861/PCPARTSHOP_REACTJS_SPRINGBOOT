...
const UpdateCategory = ({category, hide}) => {
  ...
  const onCreateCategory = async(data) => {
    ...
    console.log("ss" + data.active)
    dispatch(updateCategoryStart({id: category.id, data: formData, hide}));
  };
      ...
      <div className="flex flex-col items-center p-5 gap-5 ">
        <form onSubmit= {handleSubmit(onCreateCategory)} className='flex gap-3 mb-10 w-[600px]' encType='multipart/form-data'>
          <fieldset className='py-10 flex flex-col justify-center items-center px-6 bg-white drop-shadow-md shadow-inner rounded-xl gap-10 text-black'>
            <div className='flex flex-col items-center gap-2'>
              <img src={selectedImage ? selectedImage : "http://localhost:8080/images/setting/default-product"} alt="Avatar"
                className='w-[150px] h-[150px] shadow rounded-md overflow-hidden text-center align-middle'>
              </img>
              <input
            ...
            <div className='flex justify-center gap-2 items-baseline w-full'>
              ...
              <div className=' flex justify-start gap-2 items-center self-start w-full'>
                ...
                <select name="active"
                  id="active"
                  data-te-select-init
                  className='input w-[60%]' defaultValue={category.active} {...register("active")}>
          ...
          <div className='gap-2 fixed bottom-0 w-full bg-slate-700 p-2 right-0 flex justify-end '>
            <div className='cursor-pointer middle none center mr-4 rounded-lg bg-red-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none'
             onClick={() => hide()}>
              Hủy</div>
            <button type="submit" className='middle none center mr-4 rounded-lg bg-green-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none'>
              Lưu</button>
          </div>
        </form>
      </div>
