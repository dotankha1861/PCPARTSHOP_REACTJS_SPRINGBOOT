...
import { createCategoryStart, deleteCategoryStart, updateCategoryStart } from '../../../redux/slices/categorySlice';
import { updateCategory } from '../../../redux/apis/categoryApi';
...
const UpdateProduct = ({ product, hide }) => {
  ...
  const getProductDetail = async () => {
    try {
      const { data: resBody } = await getProductBySkuCode(product.skuCode);
      setProductDetail(resBody.data);
    }
    catch (error) {
  ...
  const onUpdateProduct = async (data) => {
    ...
    dispatch(updateProductStart({productId: product.id, data: formData, hide: () => setReadOnlyInput(true)}))
  };
  ...
  useEffect(() => {
    setValue("detailDesc", productDetail?.detailDesc);
    setValue("shortDesc", productDetail?.shortDesc);
    setValue("categoryId", productDetail?.categoryId);
    setValue("active", productDetail?.active);
    setValue("skuCode", productDetail?.skuCode);
  ...
  return (
    <div className="flex flex-col items-center p-5 gap-5" >
      <form onSubmit={handleSubmit(onUpdateProduct)} className='flex gap-3 mb-10' encType='multipart/form-data'>
        ...
        <fieldset className='w-[900px] py-10 flex flex-col justify-center items-center px-6 bg-white drop-shadow-md shadow-inner rounded-xl gap-4 text-black'
          ...
          <div className='flex gap-10 items-center w-full justify-start'>
            ...
            <div className='w-[50%] flex gap-7 items-baseline'>
            ...
            <div className='w-[50%] flex justify-start gap-2 items-start self-start'>
              ...
              <div className="flex flex-col w-full">
                <select name="categoryId"
                  ...
                  {productStatus.map(item => <option key={item.value} value={item.value} className='block'  >{item.name}</option>)}
                </select>
                <div className="text-red-500 text-sm mt-1 font-serif"><pre className='font-serif'>    {errors.firstName?.message}</pre></div>
              </div>
            </div>
          ...
          <div className='w-[100%] flex flex-col justify-start gap-2 items-start'>
            ...
            <div className="flex flex-col w-full">
              <textarea type='email'
                ...
                spellCheck={false}
                {...register("shortDesc")}
                defaultValue={productDetail?.shortDesc}>
              </textarea>
          ...
          <div className='w-[100%] flex flex-col justify-start gap-2 items-start '>
            ...
            <div className="flex flex-col w-full ">
              <ReactQuill
                ...
                name='detailDesc'
                value={editorContent}
                defaultValue={productDetail?.detailDesc}
        ...
        {readOnlyInput ? <div className='gap-2 fixed bottom-0 w-full bg-slate-700 p-2 right-0 flex justify-end '>
