...
import { cancelPromotionStart } from '../../../redux/slices/promotionSlice';
// import { deleteCategoryStart } from '../../../redux/slices/categorySlice';

const CancelPromotion = ({promotion, hide}) => {
  const dispatch = useDispatch();
  const handleCancelPromotion = () => {
    dispatch(cancelPromotionStart({id: promotion.id, hide}));
  }

  return (
    <div className='py-5 px-20 flex flex-col gap-3 justify-start items-center shadow-md overflow-hidden'>
          <div>Bạn chắc chắn muốn hủy chương trình khuyến mãi <span className='font-semibold'>#{promotion.id} - {promotion.name} ?</span></div>
