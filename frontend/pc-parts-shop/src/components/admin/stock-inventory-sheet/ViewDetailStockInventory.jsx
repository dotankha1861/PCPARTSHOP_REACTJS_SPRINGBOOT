...
import { getSISheetDetail } from '../../../redux/apis/siSheetApi';
...
const ViewDetailSISheet = ({ siSheet, hide }) => {
    const [listProduct, setListProduct] = useState([]);

    useEffect(() => {
        const getDetails = async () => {
            try {
                const { data: resBody } = await getSISheetDetail(siSheet.id);
                setListProduct(resBody.data);
            }
            catch (error) {
                //
            }
    ...
    return (
        <div className="flex items-start py-1 pb-5 pl-2 pr-2 gap-5">
            <div className='w-[850px] min-h-screen flex mb-10' encType='multipart/form-data'>

                <fieldset className='py-10 flex flex-col items-center px-6 bg-white drop-shadow-md shadow-inner rounded-xl gap-5 text-black w-full h-full'>
                <h1 className="text-xl font-bold ">THÔNG TIN PHIẾU KIỂM</h1>
                <div className='flex items-start justify-start w-full flex-col'>
                    <div>Mã phiếu kiểm: {siSheet.id}</div>
                    <div>Thời gian tạo: {siSheet.createdAt}</div>
                    <div>Nhân viên tạo: {siSheet.employeeName} </div>

                </div>
                    <h1 className="text-xl font-bold self-start">DANH SÁCH SẢN PHẨM KIỂM</h1>
                    {
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th class="px-6 py-3 w-[100px]">
                            ...
                            <tbody>
                                {
                                    listProduct.map((item, index) => <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 h-[80px]">
                                        ...
                                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white whitespace-pre-wrap w-[400px]">
                                            {item.productName}
                                        </td>
                                        <td class="px-6 py-4 w-[100px]">
                                            <div id="first_product" class="text-center justify-center bg-gray-50 w-14  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="1" >{item.initialQuantity}</div>
                                        </td>
                                        <td class="m-auto px-6 py-4 font-semibold text-gray-900 dark:text-white w-[100px]">
                                            <div className='text-center'>{item.actualQuantity}</div>
                                        </td>
                                        <td className="w-[100px] text-center px-6 py-4 font-semibold text-gray-900 dark:text-white">{item.actualQuantity - item.initialQuantity}</td>
                            ...
                            {listProduct.length > 0 && <tfoot class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center w-[100px]  whitespace-nowrap">
                                        {listProduct.reduce((total, product) => total + product.initialQuantity, 0)}
                                    </th>
                                    <th scope="col" class="px-6 py-3 w-[150px] whitespace-nowrap text-center flex justify-center">
                                        {listProduct.reduce((total, product) => total + Number(product.actualQuantity), 0)}
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-center w-[100px] whitespace-nowrap">
                                        {listProduct.reduce((total, product) => total + (product.actualQuantity - product.initialQuantity), 0)}
                                    </th>
...
export default ViewDetailSISheet;