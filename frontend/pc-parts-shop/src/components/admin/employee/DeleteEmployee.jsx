...
const DeleteEmployee = ({employee, hide}) => {
  const dispatch = useDispatch();

  const handleDeleteEmployee= () => {
    dispatch(deleteEmployeeStart({employeeId: employee.employeeId,  hide}));
  }

  return (
    <div className='py-5 px-20 flex flex-col gap-3 justify-center items-center shadow-md overflow-hidden w-[600px]'>
          <div className='text-center'>Bạn chắc chắn muốn xóa nhân viên <span className='font-semibold '>#{employee.employeeId} - {employee.lastName + " " + employee.firstName} ?</span></div>
